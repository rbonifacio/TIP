package br.unb.cic.tip.intraprocedural.pointer

import br.unb.cic.tip.pointer.BasicAndersen
import br.unb.cic.tip.utils.{AssignmentStmt, FunDecl, SequenceStmt}
import br.unb.cic.tip.utils.Expression.*
import org.scalatest.funsuite.AnyFunSuite

class BasicAndersenTest extends AnyFunSuite {

  /**
   *  s1: a = 1
   *  s2: b = alloc null
   *  s3: c = &b
   *  s4: d = c
   *  s5: e = *c
   *  s6: *b = d
   *  s7: f = null
   *
   */
  test("test_rd_calling_function_one_time") {

    val s1 = AssignmentStmt("a", ConstExp(1))
    val s2 = AssignmentStmt("b", AllocExp(NullExp))
    val s3 = AssignmentStmt("c", LocationExp("b"))
    val s4 = AssignmentStmt("d", PointerExp("b"))
    val s5 = AssignmentStmt("e", LoadExp(PointerExp("c")))
    val s7 = AssignmentStmt("g", NullExp)
    //main function7u
    val mainBody = SequenceStmt(s1, SequenceStmt(s2, SequenceStmt(s3, SequenceStmt(s4, SequenceStmt(s5, s7)))))

    val mainFunction = FunDecl("main", List(), List(), mainBody, NullExp)

//    val program = List(mainFunction)

    val RD = BasicAndersen.run(mainBody)

//    println(RD)

    assert(RD(VariableExp(s1.name)) == (
      Set(ConstExp(1))
      )
    )

    assert(RD(VariableExp(s2.name)) == (
      Set(AllocExp(NullExp))
      )
    )

    assert(RD(VariableExp(s3.name)) == (
      Set(VariableExp("b"))
      )
    )

    assert(RD(VariableExp(s4.name)) == (
      Set(AllocExp(NullExp))
      )
    )

    assert(RD(VariableExp(s5.name)) == (
      Set(AllocExp(NullExp))
      )
    )

    assert(RD(VariableExp(s7.name)) == (
      Set()
      )
    )
  }
}
