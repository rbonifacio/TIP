...
//  }
  ...
  test("final of sequence for if else stmt") {
    val s1 = AssignmentStmt("x", ConstExp(1))
    val s2 = AssignmentStmt("y", ConstExp(1))
    val s3 = AssignmentStmt("z", DirectFunctionCallExp("suma", List[Expression]()))
    val s4 = OutputStmt(VariableExp("z"))
    val seq = SequenceStmt(s1, SequenceStmt(s2, SequenceStmt(s3, s4)))
